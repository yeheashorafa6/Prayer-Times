...
import Prayer from "./component/Pray/Prayer";
...
function MainContent() {
  const { timings } = useContext(PrayerContext);
  // STATE
  ...
  const setupCountDownTimer = () => {
    ...
    if (remineTime < 0) {
      ...
      const fajrTomidNightDiff =newPrayerTimeMoment.diff( moment("00:00:00" , "hh:mm:ss"));
    ...
    setRemaingTime(`${durationRemeingTime._data.hours} : ${durationRemeingTime._data.minutes} :${durationRemeingTime._data.seconds}`)
    ...
    <>
      ...
      >
        <Grid item xs={6} className="item">
          <div className="">
            <h2 className="date">
              <FontMedia>{today}</FontMedia>
            </h2>
            <h1 className="place">
              <FontMedia>{selectCity}</FontMedia>
            </h1>
          </div>
        </Grid>
        <Grid item xs={6} className="item">
          <div className="">
            <FontMedia>
              <h2 className="remain">
                <FontMedia>Remains until the {nextPrayerTimings} prayer </FontMedia>
              </h2>
              <h1 className="time">
                <FontMedia style={{direction : "ltr"}}>{remaingTime}</FontMedia>
              </h1>
            </FontMedia>
          </div>
        </Grid>
